...
if not os.path.isfile(os.path.join(os.getcwd(), "Model/Triple_GAN_RNN_G1_D2_C1_control/Triple_GAN_RNN_G1_D2_C1_control.ckpt.meta")):
    with tf.Session(graph=graph) as session:
        tf.logging.set_verbosity(tf.logging.DEBUG)
        tf.global_variables_initializer().run()
        tf.local_variables_initializer().run()
        saver = tf.train.Saver()
        progress = open(os.path.join(os.getcwd(), 'Model/Triple_GAN_RNN_G1_D2_C1_control/Triple_GAN_RNN_G1_D2_C1_control.txt'), 'w')
        for step in range(EPOCH):
            ...
            if step % DISPLAY_EPOCH == 0:
                ...
                progress.write('\n')
                saver.save(session,
                           os.path.join(os.getcwd(), "Model/Triple_GAN_RNN_G1_D2_C1_control/Triple_GAN_RNN_G1_D2_C1_control_"+str(step)+".ckpt"))

        saver.save(session, os.path.join(os.getcwd(), "Model/Triple_GAN_RNN_G1_D2_C1_control/Triple_GAN_RNN_G1_D2_C1_control.ckpt"))
    ...
    with tf.Session(graph=graph) as session:
        ...
        saver.restore(session, os.path.join(os.getcwd(), 'Model/Triple_GAN_RNN_G1_D2_C1_control/Triple_GAN_RNN_G1_D2_C1_control.ckpt'))
