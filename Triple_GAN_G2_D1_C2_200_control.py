...
def generator(inputs, conditional, variables, reuse=False):
    ...
    return tf.nn.sigmoid(tf.matmul(rnn2_outputs[-1], variables['gen_w1_lstm']) + variables['gen_b1_lstm'])
...
def classifier(inputs, variables, reuse=False):
    ...
    return tf.matmul(rnn2_outputs[-1], variables['cls_w1_lstm']) + variables['cls_b1_lstm']
...
with graph.as_default():
    ...
    cls_loss = cls_R_L + alpha_p * cls_R_P + (1 - gen_control) * cls_disc_loss

    gen_loss = gen_control * tf.reduce_mean(
...
if not os.path.isfile(os.path.join(os.getcwd(), "Model/Triple_GAN_RNN_G2_D1_C2_200_control/Triple_GAN_RNN_G2_D1_C2_200_control.ckpt.meta")):
    with tf.Session(graph=graph) as session:
        tf.logging.set_verbosity(tf.logging.DEBUG)
        tf.global_variables_initializer().run()
        tf.local_variables_initializer().run()
        saver = tf.train.Saver()
        progress = open(os.path.join(os.getcwd(), 'Model/Triple_GAN_RNN_G2_D1_C2_200_control/Triple_GAN_RNN_G2_D1_C2_200_control.txt'), 'w')
        for step in range(EPOCH):
            ...
            if step % DISPLAY_EPOCH == 0:
                ...
                progress.write('\n')
                saver.save(session,
                           os.path.join(os.getcwd(), "Model/Triple_GAN_RNN_G2_D1_C2_200_control/Triple_GAN_RNN_G2_D1_C2_200_control_"+str(step)+".ckpt"))

        saver.save(session, os.path.join(os.getcwd(), "Model/Triple_GAN_RNN_G2_D1_C2_200_control/Triple_GAN_RNN_G2_D1_C2_200_control.ckpt"))
    ...
    with tf.Session(graph=graph) as session:
        ...
        saver.restore(session, os.path.join(os.getcwd(), 'Model/Triple_GAN_RNN_G2_D1_C2_200_control/Triple_GAN_RNN_G2_D1_C2_200_control.ckpt'))
