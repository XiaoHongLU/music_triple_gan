...
def model(inputs, variables):
    ...
    # Backward direction cell
        ...
        outputs_1, _, _ = rnn.static_bidirectional_rnn(lstm_fw_cell_1, lstm_bw_cell_1, outputs, dtype=tf.float32, scope=scope)
    ...
    # Backward direction cell
        ...
        outputs_2, _, _ = rnn.static_bidirectional_rnn(lstm_fw_cell_2, lstm_bw_cell_2, outputs_1, dtype=tf.float32, scope=scope)
...
if not os.path.isfile(os.path.join(os.getcwd(),"Model/3RNN/3RNN.ckpt.meta")):
    with tf.Session(graph=graph) as session:
        tf.logging.set_verbosity(tf.logging.DEBUG)
        tf.global_variables_initializer().run()
        tf.local_variables_initializer().run()
        progress = open(os.path.join(os.getcwd(), 'Model/3RNN/3RNN.txt'), 'wb')
        ...
        saver.save(session, os.path.join(os.getcwd(), "Model/3RNN/3RNN.ckpt"))
else:
    with tf.Session(graph=graph) as session:
        saver = tf.train.Saver()
        tf.global_variables_initializer().run()
        ...
        saver.restore(session, os.path.join(os.getcwd(), 'Model/3RNN/3RNN.ckpt'))
