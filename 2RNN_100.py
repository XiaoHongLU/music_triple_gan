...
def model(inputs, variables):
    ...
    # Forward direction cell
        lstm_fw_cell2 = rnn.BasicLSTMCell(HIDDEN_DIMENSION, forget_bias=1.0)
    # Backward direction cell
        lstm_bw_cell2 = rnn.BasicLSTMCell(HIDDEN_DIMENSION, forget_bias=1.0)

        outputs2, _, _ = rnn.static_bidirectional_rnn(lstm_fw_cell2, lstm_bw_cell2, outputs, dtype=tf.float32, scope=scope1)

    return tf.nn.sigmoid(tf.matmul(outputs2[-1], variables['w1_lstm']) + variables['b1_lstm'])
...
if not os.path.isfile(os.path.join(os.getcwd(),"Model/2RNN_100/2RNN_100.ckpt.meta")):
    with tf.Session(graph=graph) as session:
        ...
        saver.save(session, os.path.join(os.getcwd(), "Model/2RNN_100/2RNN_100.ckpt"))
else:
    with tf.Session(graph=graph) as session:
        saver = tf.train.Saver()
        tf.global_variables_initializer().run()
        ...
        saver.restore(session, os.path.join(os.getcwd(), 'Model/2RNN_100/2RNN_100.ckpt'))
