...
def model(inputs, variables):
    ...
    with tf.variable_scope('layer2') as scope1:
    # Forward direction cell
    ...
    # Backward direction cell
        ...
        outputs_1, _, _ = rnn.static_bidirectional_rnn(lstm_fw_cell_1, lstm_bw_cell_1, outputs, dtype=tf.float32, scope=scope1)

    with tf.variable_scope('layer3') as scope2:
    # Forward direction cell
    ...
    # Backward direction cell
        ...
        outputs_2, _, _ = rnn.static_bidirectional_rnn(lstm_fw_cell_2, lstm_bw_cell_2, outputs_1, dtype=tf.float32, scope=scope2)

    return tf.nn.sigmoid(tf.matmul(outputs_2[-1], variables['w1_lstm']) + variables['b1_lstm'])
...
with graph.as_default():
    ...
    model_ = model
    logits = model_(tf_train_data, variables)

    loss = tf.losses.sigmoid_cross_entropy(tf_train_label, logits)
    acc = acc = tf.reduce_mean(tf.cast(tf.equal(tf.reduce_mean(tf.cast(tf.equal(tf_train_label, logits), dtype=tf.float32),
                                                         axis=1), 1), dtype=tf.float32))

    optimizer = tf.train.AdamOptimizer(LEARNING_RATE).minimize(loss)

if not os.path.isfile(os.path.join(os.getcwd(),"Model/3RNN_100/3RNN_100.ckpt.meta")):
    with tf.Session(graph=graph) as session:
        tf.logging.set_verbosity(tf.logging.DEBUG)
        tf.global_variables_initializer().run()
        tf.local_variables_initializer().run()
        progress = open(os.path.join(os.getcwd(), 'Model/3RNN_100/3RNN_100.txt'), 'wb')
        ...
        saver.save(session, os.path.join(os.getcwd(), "Model/3RNN_100/3RNN_100.ckpt"))
else:
    with tf.Session(graph=graph) as session:
        saver = tf.train.Saver()
        tf.global_variables_initializer().run()
        ...
        saver.restore(session, os.path.join(os.getcwd(), 'Model/3RNN_100/3RNN_100.ckpt'))
