...
if not os.path.isfile(os.path.join(os.getcwd(),"Model/RNN/RNN.ckpt.meta")):
    with tf.Session(graph=graph) as session:
        tf.logging.set_verbosity(tf.logging.DEBUG)
        tf.global_variables_initializer().run()
        tf.local_variables_initializer().run()
        progress = open(os.path.join(os.getcwd(), 'Model/RNN/RNN.txt'), 'wb')
        ...
        saver.save(session, os.path.join(os.getcwd(), "Model/RNN/RNN.ckpt"))
else:
    with tf.Session(graph=graph) as session:
        saver = tf.train.Saver()
        tf.global_variables_initializer().run()
        ...
        saver.restore(session, os.path.join(os.getcwd(), 'Model/RNN/RNN.ckpt'))
